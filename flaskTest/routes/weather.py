...
def get_weather2():
    print("GET 요청을 받았습니다.")
    # nx_val = requests.args.get('x', default=None, type=None)
    # ny_val = requests.args.get('y', default=None, type=None)
    # print("X : ", nx_val)
    # print("Y : ", ny_val)
    ...
    req_parameter = {'serviceKey': api.API_KEY_decode,
                     'nx': nx_val, 'ny': ny_val,
    ...
    json_weather = f'<span style="color:blue">hum</span> <span> {weather_data.get("hum")}</span><br/>'
    json_weather += f'<span style="color:blue">pre</span> <span> {weather_data.get("pre")}</span><br/>'
    json_weather += f'<span style="color:blue">tmp</span> <span> {weather_data.get("tmp")}</span><br/>'
    return json_weather
